...
function SignIn({ setToken, setUsername }) {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const navigate = useNavigate();

  // Optional admin hardcoded check (you can remove this if backend handles admin)
  ...
  const handleSignIn = async (e) => {
    ...
    if (email === adminEmail && password === adminPassword) {
      ...
      localStorage.setItem('token', 'admin_dummy_token'); // or handle differently
      localStorage.setItem('email', email);
      setToken('admin_dummy_token');
      setUsername(email);
